...
TEST(config_user_key_length_is_strict)
{
    mqvpn_config_t *cfg = mqvpn_config_new();
    ASSERT_NOT_NULL(cfg);
    ASSERT_EQ(mqvpn_config_add_user(cfg, "alice", "original-key"), MQVPN_OK);
    ASSERT_EQ(mqvpn_config_add_route(cfg, "alice", "10.50.0.0/16"), MQVPN_OK);
    char key[257], json[400];
    memset(key, 'k', sizeof(key) - 1);
    key[sizeof(key) - 1] = '\0';
    ASSERT_EQ(mqvpn_config_add_user(cfg, "alice", key), MQVPN_ERR_INVALID_ARG);
    snprintf(json, sizeof(json), "{\"users\":[{\"name\":\"alice\",\"key\":\"%s\"}]}", key);
    ASSERT_EQ(mqvpn_config_load_json(cfg, json), MQVPN_ERR_INVALID_ARG);
    ASSERT_STR_EQ(cfg->user_keys[0], "original-key");
    ASSERT_EQ(cfg->n_routes, 1);
    snprintf(json, sizeof(json), "{\"users\":[\"alice:%s\"]}", key);
    ASSERT_EQ(mqvpn_config_load_json(cfg, json), MQVPN_ERR_INVALID_ARG);
    ASSERT_STR_EQ(cfg->user_keys[0], "original-key");
    ASSERT_EQ(cfg->n_routes, 1);
    mqvpn_config_free(cfg);
...
TEST(config_set_sync_path_labels)
{
    mqvpn_config_t *cfg = mqvpn_config_new();
    ASSERT_NOT_NULL(cfg);
    ASSERT_EQ(cfg->sync_path_labels, 1); /* default true */
    ASSERT_EQ(mqvpn_config_set_sync_path_labels(NULL, 1), MQVPN_ERR_INVALID_ARG);
    ASSERT_EQ(mqvpn_config_set_sync_path_labels(cfg, 0), MQVPN_OK);
    ASSERT_EQ(cfg->sync_path_labels, 0);
    ASSERT_EQ(mqvpn_config_set_sync_path_labels(cfg, 5), MQVPN_OK);
    ASSERT_EQ(cfg->sync_path_labels, 1); /* nonzero normalizes to 1 */
...
main(void)
{
    ...
    run_config_user_key_length_is_strict();
